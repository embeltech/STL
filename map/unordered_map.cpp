...
// unordered_map-> unique keys and not sorted keys
//If new key value pair is added which has same key the mapped value is overwritten with new value

void printMap(const std::unordered_map<std::string,std::string>& mapData)
{
    std::cout<<"----------------------------\n";
...
int main()
{
    std::unordered_map<std::string,std::string> mPhonebook;
    ...
    std::string strName("PQR"), strNum("3333344444");
